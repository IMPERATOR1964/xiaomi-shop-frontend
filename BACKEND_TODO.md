...
## 11. Полное удаление товара (hard delete)

**Запрос пользователя**: в админке у товара есть только «Изменить» и «Скрыть».
Нужно настоящее удаление записи, а не soft delete (`isActive=false`).

**Сейчас**: `DELETE /api/products/{id}` — это soft delete (выставляет `isActive=false`),
плюс есть `POST /api/products/{id}/restore`. Полного удаления нет.

**Нужно**: новый эндпоинт
```
DELETE /api/admin/products/{id}   → 204 No Content   (ROLE_ADMIN)
```
- Удаляет товар физически (и связанные строки: фото галереи, варианты,
  значения атрибутов и т.п. — каскадом или вручную).
- Если товар фигурирует в заказах (`order_items`) — **не удалять**, вернуть
  **409 Conflict** (фронт покажет «товар используется в заказах, используйте Скрыть»).
- Доступ строго ADMIN (не MODERATOR).

**Фронт уже готов** (`productsApi.hardDelete` → `DELETE /api/admin/products/{id}`,
кнопка «Удалить» в `AdminProductsPage.jsx`). Пока эндпоинта нет — клик показывает
сообщение «полное удаление пока не поддерживается». На `409` показывает про заказы.
